...
const options = [
  { name: "routes", type: "Record<string, string>", desc: "Named, server-controlled destinations so the client never picks the URL." },
  { name: "baseUrl", type: "string", desc: "Prefix used to resolve relative endpoints." },
  { name: "allowOrigins", type: "string[]", desc: "CORS whitelist of permitted origins." },
  { name: "allowPrivateHosts", type: "boolean", desc: "Opt-in escape hatch for internal hosts (off by default)." },
  { name: "inMemoryRate", type: "{ windowMs, max, key? }", desc: "Simple in-memory rate limiting grouped by IP or custom key." },
  { name: "transformRequest", type: "({ method, endpoint, data }) => {…}", desc: "Modify the payload before the upstream fetch." },
  { name: "transformResponse", type: "(res) => any", desc: "Adjust the response before sending it to the client." },
  { name: "maskSensitiveData", type: "(data) => any", desc: "Sanitize and mask sensitive keys before transit." },
  { name: "validate", type: "(req) => boolean | Promise", desc: "Block the flow for auth or permission checks." },
  { name: "log", type: "(info) => void", desc: "Receive request, response, and error events." },
]
        ...
        <div className="mt-12 grid gap-10 lg:grid-cols-2">
          ...
          <div className="overflow-hidden rounded-2xl border border-border lg:order-1">
            <div className="border-b border-border bg-card/60 px-5 py-3">
              <h3 className="font-mono text-sm font-semibold">Configuration options</h3>
            </div>
            <div className="max-h-[460px] overflow-y-auto divide-y divide-border/60">
              {options.map((o) => (
                <div key={o.name} className="px-5 py-4 transition-colors hover:bg-secondary/40">
                  <div className="flex flex-wrap items-center gap-2">
                    <code className="rounded bg-primary/12 px-1.5 py-0.5 font-mono text-xs text-primary">
                      {o.name}
                    </code>
                    <code className="font-mono text-xs text-muted-foreground/80">{o.type}</code>
                  </div>
